...
def register_user():
    ...
    registration_dict.update(
        {"registration-creation-device": user_agent.platform})
    try:
        registration_dict.update(
            {"registration-creation-language": request.accept_languages[0][0]})
    except Exception as e:
        registration_dict.update({"registration-creation-language": None})

    try:
        browser_dict = {"browser": user_agent.browser,
                        "version": user_agent.version}
    except Exception as e:
        browser_dict = {"browser": None, "version": None}

    registration_dict.update(
        {"registration-creation-browser-info": browser_dict})
...
def log_login_attempt():
    ...
    user_agent = request.user_agent
    login_object = UILC()
    status_dict = login_object.login_user(
        login_dict['user-id'], login_dict['user-password'])
    browser_dict = {"browser": user_agent.browser,
                    "version": user_agent.version}
    login_dict.update({"login-browser-info": browser_dict, "activity-info": {
                      "login-success": status_dict["login-success"], "invalid-user-id": status_dict["invalid-user-id"], "invalid-pwd": status_dict["invalid-pwd"]}})
